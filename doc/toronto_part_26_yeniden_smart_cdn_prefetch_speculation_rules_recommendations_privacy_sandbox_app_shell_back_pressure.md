...
export default async function LocaleLayout({ children }:{ children: React.ReactNode }){
  const h = headers();
  const lang = h.get('x-locale') || 'tr';
  const device = /mobile/i.test(h.get('user-agent')||'') ? 'm' : 'd';
  const saveData = h.get('save-data') === 'on';
  return (
    <html lang={lang}>
      <body>
        {!saveData && <Speculation device={device as 'm'|'d'} lang={lang as 'tr'|'de'|'en'} />}
        {children}
      </body>
    </html>
  );
...
import { getTopRoutes } from '@/shared/perf/topRoutes'; // son 7 gün CTR>3% ilk 5 sayfa
export default async function Speculation({ device, lang }:{ device:'m'|'d'; lang:'tr'|'de'|'en' }){
  const routes = await getTopRoutes({ device, lang });
  const entries = routes.map(p=> ({ source: 'document', where: { href_matches: `${process.env.NEXT_PUBLIC_ORIGIN}/${lang}${p}` }, eagerness: 'moderate' }));
  const json = { prerender: entries.slice(0,2), prefetch: entries.slice(2,6) };
