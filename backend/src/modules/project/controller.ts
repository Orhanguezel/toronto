...
/** GET BY SLUG (public) */
export const getProjectBySlugPublic: RouteHandler<{ Params: { slug: string } }> = async (req, reply) => {
  const row = await getProjectMergedBySlug((req as any).locale, DEFAULT_LOCALE, req.params.slug);
  if (!row) return reply.code(404).send({ error: { message: "not_found" } });
  return reply.send(row);
};

/** LIST IMAGES of a project (public, coalesced) */
export const listProjectImagesPublic: RouteHandler<{ Params: { id: string } }> = async (req, reply) => {
