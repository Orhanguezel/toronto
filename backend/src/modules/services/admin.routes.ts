...
export async function registerServicesAdmin(app: FastifyInstance) {
  ...
  app.get(`${BASE}/by-slug/:slug`, { config: { auth: true } }, getServiceBySlugAdmin);
  ...
  // gallery
  app.get(`${BASE}/:id/images`,             { config: { auth: true } }, listServiceImagesAdmin);
  app.post(`${BASE}/:id/images`,            { config: { auth: true } }, createServiceImageAdmin);
  app.patch(`${BASE}/:id/images/:imageId`,  { config: { auth: true } }, updateServiceImageAdmin);
  app.delete(`${BASE}/:id/images/:imageId`, { config: { auth: true } }, removeServiceImageAdmin);
}
