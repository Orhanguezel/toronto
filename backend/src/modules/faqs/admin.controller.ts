...
export const getFaqAdmin: RouteHandler<{ Params: { id: string } }> = async (req, reply) => {
  const row = await getFaqMergedById((req as any).locale, DEFAULT_LOCALE, req.params.id);
...
export const getFaqBySlugAdmin: RouteHandler<{ Params: { slug: string } }> = async (req, reply) => {
  const row = await getFaqMergedBySlug((req as any).locale, DEFAULT_LOCALE, req.params.slug);
  if (!row) return reply.code(404).send({ error: { message: "not_found" } });
  return reply.send(row);
};
 ...
 *  - Header/body locale’e göre i18n satırını oluşturur
 */
export const createFaqAdmin: RouteHandler<{ Body: UpsertFaqBody }> = async (req, reply) => {
  ...
  try {
    ...
    const row = await getFaqMergedById(locale, DEFAULT_LOCALE, id);
...
export const updateFaqAdmin: RouteHandler<{ Params: { id: string }; Body: PatchFaqBody }> = async (req, reply) => {
  ...
  try {
    ...
    const row = await getFaqMergedById(locale, DEFAULT_LOCALE, req.params.id);
    if (!row) return reply.code(404).send({ error: { message: "not_found" } });
    return reply.send(row);
  } catch (err: any) {
    if (err?.code === "ER_DUP_ENTRY") {
