...
CREATE TABLE IF NOT EXISTS `reference_images` (
  ...
  CONSTRAINT fk_reference_images_asset
...
SET @REFIMG_TORONTO_A_ID := (
  ...
  WHERE reference_id=@REF_TORONTO_ID AND asset_id=@ASSET_G1A_ID
  LIMIT 1
);
...
ON DUPLICATE KEY UPDATE
 asset_id=VALUES(asset_id), display_order=VALUES(display_order), is_active=VALUES(is_active), updated_at=VALUES(updated_at);

-- Toronto: image B
SET @REFIMG_TORONTO_B_ID := (
  SELECT id FROM reference_images
  WHERE reference_id=@REF_TORONTO_ID AND asset_id=@ASSET_G1B_ID
  LIMIT 1
);
...
SET @REFIMG_ECOM_A_ID := (
  SELECT id FROM reference_images
  WHERE reference_id=@REF_ECOM_ID AND asset_id=@ASSET_G2A_ID
  LIMIT 1
);
